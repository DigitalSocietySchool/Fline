!function($){
	jQuery.fn.extend({html5_qrcode:function(qrcodeSuccess,qrcodeError,videoError){
		return this.each(function(){
			var currentElem=$(this),
			height=currentElem.height(),
			width=currentElem.width();
			null==height&&(height=250),
			null==width&&(width=300);
			...
			scan=function(){
				if(localMediaStream){
					...
					try{
						...
						qrcodeError(e,localMediaStream)}$.data(currentElem[0],"timeout",setTimeout(scan,500))
						...
						var successCallback=function(stream){
							...
							navigator.getUserMedia?navigator.getUserMedia({video:!0},
							successCallback,function(error){videoError(error,localMediaStream)}):console.log("Native web camera streaming (getUserMedia) not supported in this browser."),qrcode.callback=function(result){qrcodeSuccess(result,localMediaStream)}})},html5_qrcode_stop:function(){return this.each(function(){$(this).data("stream").getVideoTracks().forEach(function(videoTrack){videoTrack.stop()}),clearTimeout($(this).data("timeout"))})}})}(jQuery);